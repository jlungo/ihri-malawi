...
class iHRIS_Module_Station extends I2CE_Module {
    ...
    public function pre_upgrade( $old_vers, $new_vers, $new_storage ) {
        if ( I2CE_Validate::checkVersion( $old_vers, '<', '3.2.3' ) ) {
            ...
            I2CE_FormStorage::storeMigrateData( array( "station" => 
                        array( "station_type", "country", "district", "county" ) ), 
                    $migrate_path );
        }   
    ...
    public function upgrade( $old_vers, $new_vers ) {
        ...
        if ( I2CE_Validate::checkVersion( $old_vers, '<', '3.2.3' ) ) {
            ...
            if ( !I2CE_FormStorage::migrateForm( "station_type", "entry", $user, $migrate_path, 
                        false, array( "type" ) ) ) {
                return false;
            }

            if ( I2CE_FormStorage::migrateForm( "station", "entry", $user, $migrate_path,
                        false, array( "type" ), 
                        array( "station_type" => "station_type",
                            "location" => array( "county" => "county", 
                                "district" => "district", "country" => "country" ) ) ) ) {
                unset( $class_config->iHRIS_Station->fields->country );
                unset( $class_config->iHRIS_Station->fields->district );
                unset( $class_config->iHRIS_Station->fields->county );
                unset( $class_config->iHRIS_Station->fields->type );
            } else {
                return false;
            }
            ...
            if ( !I2CE_FormStorage::migrateForm( "station", "entry", $user, $migrate_path, false, array( "type" ) ) ) {
                return false;
            }
            unset( $class_config->iHRIS_Station->fields->type );
