...
*/ 
/** 
* Class iHRIS_Module_StationContact
* 
* @access public
*/


class iHRIS_Module_StationContact extends I2CE_Module {

    /**
     * Migrate the station contact data from entry to the current storage (magicdata)
     */
    public function migrate() {
        $user = new I2CE_User( 1, false, false, false );
        $migrate_path = "/I2CE/formsData/migrate_data/3.2.3";

        if ( !I2CE_FormStorage::migrateForm( "station_contact", "entry", $user, $migrate_path ) ) { 
            return false;
        }
        return true;
    }
    ...
    protected function fixupContact() {
        $stationObj = I2CE_FormFactory::instance()->createContainer('station');
        if (!$stationObj instanceof iHRIS_Station) {
            I2CE::raiseError("Bad station form");
            return false;
        }
        $childForms = $stationObj->getChildForms();
        if (!in_array('contact',$childForms)) {
            return true;
        }
        I2CE::raiseError("Changing contact child forms of station to station_contact");
        if (! iHRIS_Module_Contact::changeContactForm('station', 'contact_type|station','station_contact',true)) {
