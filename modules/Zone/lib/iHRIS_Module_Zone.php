...
class iHRIS_Module_Zone extends I2CE_Module {
    ...
    public function pre_upgrade( $old_vers, $new_vers, $new_storage ) {
        if ( I2CE_Validate::checkVersion( $old_vers, '<', '3.2.3' ) ) {
            ...
            I2CE_FormStorage::storeMigrateData( array("zone" => array( "county" ) ),
                    $migrate_path );
        }
...
/*
            ...
            if ( !I2CE_FormStorage::migrateForm( "village", "entry", $user, $migrate_path, 
                    false, array( "type", "district", "region", "country" ), array( "county" => "county" ) ) ) {
                return false;
            }
 */
            if ( !I2CE_FormStorage::migrateForm( "zone", "entry", $user, $migrate_path, 
                    false, array( "type", "country" ), array( "region" => "region" ) ) ) {
                return false;
            }
            ...
            */
            unset( $class_config->iHRIS_Zone->fields->type );
            unset( $class_config->iHRIS_Zone->fields->zone );
            unset( $class_config->iHRIS_Zone->fields->region );
            unset( $class_config->iHRIS_Zone->fields->country );
        }
